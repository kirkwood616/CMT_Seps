function DTF_Names_CutProof_Metadata() {
  // Active Document
  ...
  // ORDER #
  var panel_ORDER = dialog.add("panel", undefined, undefined, {
    name: "panel_ORDER",
  });
  ...
  // ART #
  var panel_ART = dialog.add("panel", undefined, undefined, {
    name: "panel_ART",
  });
  panel_ART.text = "ART #";
  ...
  // DUE DATE
  var panel_DUE = dialog.add("panel", undefined, undefined, {
    name: "panel_DUE",
  });
  ...
  okButton.onClick = function () {
    ...
    if (input_ORDER.text && input_ART.text && input_DUE.text) {
      for (var i = 0; i < metadataFrames.length; i++) {
        if (metadataFrames[i].name === "ORDER_NUMBER")
          metadataFrames[i].contents = input_ORDER.text;
        if (metadataFrames[i].name === "ART_NUMBER")
          metadataFrames[i].contents = input_ART.text.toUpperCase();
        if (metadataFrames[i].name === "DUE_DATE")
          metadataFrames[i].contents = input_DUE.text;
      }
      dialog.close();
    }
  };
...
try {
  if (
    ...
    app.activeDocument.artboards[0].name.indexOf("CP_") !== -1
  ) {
    DTF_Names_CutProof_Metadata();
  } else {
